 ...
 * - Mot de passe actuel
 * - Nouveau mot de passe (avec PasswordStrengthChecker)
 * - Confirmation du nouveau mot de passe
 * 
 * Chaque champ dispose d'un toggle de visibilité (œil ouvert/fermé).
 * Le bouton "Valider" est désactivé tant que :
 * - Tous les champs ne sont pas remplis
 * - Le nouveau mot de passe ne passe pas la validation de force
 * 
 * Validation côté serveur (server/routes/profile.js) :
 * - Mot de passe actuel vérifié via bcrypt
 * - Nouveau mot de passe : min 6 chars, majuscule, minuscule, chiffre, caractère spécial
 * - Nouveau ≠ ancien mot de passe
 * 
 * Props :
 * - showPasswordForm / setShowPasswordForm : contrôle la visibilité du formulaire
 * - pwForm / setPwForm : valeurs des 3 champs
 * - showPw / setShowPw : visibilité de chaque champ
 * - isNewPasswordValid / setIsNewPasswordValid : validation via PasswordStrengthChecker
 * - onSubmit : callback pour ouvrir le dialogue de confirmation
 * =============================================================================
 */
...
/** Classe CSS premium pour les boutons stylisés */
...
const PasswordSection: React.FC<PasswordSectionProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Formulaire de changement de mot de passe (affiché conditionnellement) */}
      {showPasswordForm && (
        <div className="space-y-4 max-w-md">
          {fields.map(({ key, label, show, toggle }) => (
            <div key={key}>
              ...
              {/* Indicateur de force du mot de passe (uniquement pour le nouveau mot de passe) */}
